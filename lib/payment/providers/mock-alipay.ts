 ...
 * Simulates Alipay payment flow with realistic delays and status updates
...
export class MockAlipayProvider implements PaymentProvider {
  async createPayment(request: PaymentRequest): Promise<PaymentResponse> {
    const transactionId = `ALIPAY${Date.now()}${Math.random().toString(36).substring(2, 9).toUpperCase()}`
    const expiresAt = new Date(Date.now() + 5 * 60 * 1000) // 5 minutes from now

    // Generate mock QR code data (in real implementation, this would be from Alipay API)
    const qrCodeData = `https://qr.alipay.com/bax${transactionId.substring(0, 18)}`
